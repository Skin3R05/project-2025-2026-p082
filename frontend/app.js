...
let password = localStorage.getItem("ipbot_pw") || "";
let inputHistory = [],
    histIndex = -1,
    sending = false,
    openMenuEl = null;
...
function newChat(doSave = true) {
    // clear out empty chats that aren't in a folder open a fresh one
...
async function doLogin() {
    ...
    if (await verifyPw(pw)) {
        password = pw;
        localStorage.setItem("ipbot_pw", pw);
        document.getElementById("auth").classList.add("hidden");
    } else document.getElementById("auth-err").textContent = "Incorrect password.";
}
...
const overlayOpen = () => [...document.querySelectorAll(".auth, .modal")].some(m => !m.classList.contains("hidden"));
...
async function init() {
    ...
    try {
        ...
        if (cfg.needs_auth && !(await verifyPw(password))) showAuth();
    } catch (e) {}
    focusInput();
}
init();